 ...
 *  @return void
 */
void convertToKlg(VEC_INFO &vec_info, std::string &strKlgFileName)
{
    ...
    {
        std::string strAbsPathDepth = std::string(getcwd(NULL, 0)) + "/" + it->second.first;
        ...
        std::string strAbsPath = std::string(getcwd(NULL, 0)) + "/" + it->second.second;
...
int main(int argc, char* argv[])
{
    ...
    int err = parseInfoFile(strAssociation_Path, vec_info);
    if(err != 0)
    {
        fprintf(stderr, 
